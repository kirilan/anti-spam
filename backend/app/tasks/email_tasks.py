    ...
    try:
        ...
        try:
            ...
            if retry_count < len(retry_intervals):
                ...
                import logging
                logging.info(f"Inbox scan failed, retrying in {countdown}s (attempt {retry_count + 1}/{len(retry_intervals)})")
            else:
                # Log final failure
                activity_service.log_activity(
                    user_id=user_id,
    ...
    try:
        ...
        for idx, msg_data in enumerate(messages):
            ...
            if existing:
                ...
                responses_updated += 1
                logger.info(f"Re-classified existing response {existing.id}: {response_type.value} ({confidence})")
            else:
        ...
        logger.info(f"Response scan completed: {responses_created} new, {responses_updated} re-classified, {requests_updated} requests updated")
        ...
        logger.error(f"Response scan failed for user {user_id}: {type(exc).__name__}: {str(exc)}", exc_info=True)
        ...
        try:
            ...
            if retry_count < len(retry_intervals):
                ...
                logger.info(f"Retrying in {countdown}s (attempt {retry_count + 1}/{len(retry_intervals)})")
            else:
                # Log final failure
                activity_service.log_activity(
                    user_id=user_id,
...
def scan_all_users_for_responses(self):
    ...
    try:
        ...
        if retry_count < len(retry_intervals):
            countdown = retry_intervals[retry_count]
            import logging
            logging.info(f"Daily scan failed, retrying in {countdown}s ({countdown/60} min) - attempt {retry_count + 1}/{len(retry_intervals)}")
